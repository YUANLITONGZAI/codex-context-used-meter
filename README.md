...
Agent 应把这个仓库作为唯一安装源，先读取仓库里的 `README.md` 和 `AGENTS.md`，再按下面的标准路径安装。不要修改 Codex 全局配置，不要粘贴 DevTools 临时代码，不要把脚本放到其它目录作为替代安装方式。

1. 克隆或打开仓库：`https://github.com/Minghou-Lei/codex-context-used-meter`。
2. 确认仓库内 `codex-context-used-meter.js` 存在，并读取脚本里的 `SCRIPT_VERSION` 作为安装版本。
3. 把 `codex-context-used-meter.js` 复制到 `%APPDATA%\Codex++\user_scripts\codex-context-used-meter.js`；目录不存在就创建。
4. 重新注入用户脚本，或重启 Codex++ / Codex App。
5. 安装后确认目标脚本存在，并再次读取目标脚本里的 `SCRIPT_VERSION`。
6. 如果用户同时要求 Provider 余额框，继续按下一节配置；否则不要创建 Provider 配置、密钥文件或 supervisor。
...
The agent should treat this repository as the only installation source, read `README.md` and `AGENTS.md` first, then install through the standard path below. Do not modify global Codex config, do not paste temporary DevTools code, and do not place the script in another directory as an alternative install method.

1. Clone or open this repository: `https://github.com/Minghou-Lei/codex-context-used-meter`.
2. Confirm `codex-context-used-meter.js` exists in the repository, and read `SCRIPT_VERSION` from it as the installed version.
3. Copy `codex-context-used-meter.js` to `%APPDATA%\Codex++\user_scripts\codex-context-used-meter.js`; create the directory if it does not exist.
4. Reload user scripts, or restart Codex++ / Codex App.
5. After installation, confirm the target script exists and read `SCRIPT_VERSION` from the target script again.
6. If the user also asks for the Provider balance card, continue with the next section; otherwise do not create Provider config, secret files, or the supervisor.
